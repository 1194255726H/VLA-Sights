import { Select, Slider, Space, Button, Modal } from 'antd';
...
export function VideoReviewPlayer({ url, selectedEvent, seekRequestId }: VideoReviewPlayerProps) {
  ...
  const modalPlayerRef = useRef<HTMLVideoElement | null>(null);
  ...
  useEffect(() => {
    ...
    if (modalPlayerRef.current) {
      modalPlayerRef.current.playbackRate = playbackRate;
    }
  }, [playbackRate]);

  useEffect(() => {
    if (!previewOpen || !modalPlayerRef.current) return;

    modalPlayerRef.current.currentTime = playedSeconds;
    modalPlayerRef.current.playbackRate = playbackRate;
  }, [previewOpen]);
  ...
  const seekTo = (value: number) => {
    ...
    if (modalPlayerRef.current) {
      modalPlayerRef.current.currentTime = value;
    }
    setPlayedSeconds(value);
  };

  const closePreview = () => {
    const currentTime = modalPlayerRef.current?.currentTime ?? playedSeconds;
    if (playerRef.current) {
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-4">
            ...
            <Space size={8}>
              <Select
                ...
                disabled={!url}
              />
              <Button size="small" icon={<FullscreenOutlined />} onClick={() => setPreviewOpen(true)} disabled={!url}>
                全屏
              </Button>
            </Space>
          </div>
      ...
      <Modal
        ...
        styles={{ body: { height: '80vh', padding: 0, background: '#000' } }}
      >
        {url ? (
          <ReactPlayer
            ref={modalPlayerRef}
            ...
            height="100%"
            controls
            onTimeUpdate={() => setPlayedSeconds(modalPlayerRef.current?.currentTime ?? 0)}
            onDurationChange={() => setDuration(modalPlayerRef.current?.duration || 60)}
          />
        ) : null}
      </Modal>
    </>
